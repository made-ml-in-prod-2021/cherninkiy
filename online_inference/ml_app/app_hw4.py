...
def halt_callback():
    logger.debug("Entering halt procedure")
    time.sleep(TIME_HALT)
    logger.debug("Running halt command")
    os._exit(1)
...
def startup():
    ...
    threading.Thread(target=halt_callback).start()
...
if __name__ == "__main__":
    uvicorn.run("ml_app.app_hw4:app", host=PREDICTOR_HOST, port=os.getenv("PORT", PREDICTOR_PORT))